...
class IncomeExpenseTracker:
    ...
    def addIncome(self, amount, date=None):
        ...
        cur.execute(f"INSERT INTO income (year, month, income, expense, date) VALUES ({int(date[0])}, '{str(self.get_months_list()[int(date[1]) - 1])}', {float(amount)}, {float(0.00)}, '{'-'.join(date)}')")
        conn.commit()
        conn.close()
    ...
    def addExpense(self, amount, category, description, type="negative", date=None):
        ...
        cur.execute(f"INSERT INTO {self.get_months_list()[int(date[1]) - 1]} (amount, date_of_expense, category, description, date_of_registration, type) VALUES ({float(amount)}, '{"-".join(date)}', '{str(category)}', '{str(description)}', '{str(self.get_current_date())}', '{str(type)}')")
        conn.commit()
        conn.close()
        ...
        cur2.execute(f"UPDATE income SET {'expense' if type == 'negative' else 'income'}={float(new_value)} WHERE year={int(date[0])} AND month='{self.get_months_list()[int(date[1]) - 1]}'")
        conn2.commit()
        conn2.close()
    ...
    def editExpense(self, date, expense_id, amount, category, description, expense_type):
        ...
        conn = connect(os.path.join(self.__EXPENSE_DATABASE_DIRECTORY, f"{date[0]}.db"))
        cur = conn.cursor()
        cur.execute(f"UPDATE {self.get_months_list()[int(date[1]) - 1]} SET amount={amount}, category='{category}', description='{description}', type='{expense_type}' WHERE id={expense_id}")
        conn.commit()
        conn.close()
