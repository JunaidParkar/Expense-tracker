...
class Database(routes):
    ...
    def __addMainStats(self, date, amount, type):
        ...
        if dt is None:
            ...
            main_db_cursor.execute(f"UPDATE month_stats SET {'expense' if type == 'neg' else 'income'}={d} WHERE year={int(date[0])} AND month='{str(self.months[int(date[1]) - 1])}'")
            main_db.commit()
        main_db.close()
    ...
    def refreshMainStats(self):
        db_files = self.getExpenseDBPathsList()
        main_db_conn = connect(self.getRoute(RouteType.DATABASE_MAIN_DB))
        main_db_cursor = main_db_conn.cursor()
        main_db_cursor.execute("DROP TABLE IF EXISTS month_stats")
        main_db_conn.commit()
        main_db_conn.close()
        for db in db_files:
            db_conn = connect(os.path.join(self.getRoute(RouteType.DATABASE_USER_EXPENSE_DIR), db))
            ...
            for table in tables:
                ...
                for data in datas:
                    self.__addMainStats(str(data[1]).split("-"), data[2], data[3])
            db_conn.close()

    def getAllMonthStats(self):
        db_conn = connect(self.getRoute(RouteType.DATABASE_MAIN_DB))
        db_cursor = db_conn.cursor()
